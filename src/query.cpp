...
Json::Value RiotApiClient::query(std::string_view endpoint, std::string_view end_type, std::vector<std::string> params) {

    for (int i = 0; i < params.size(); i++) {
        params[i] = this->encode_url(params[i]);
    }
    
    auto query_form = this->query_types.at(endpoint).at(end_type);
    std::string address = query_form->construct_url(params);
